...
def training_and_test(token, train_data, test_data, num_classes, result, model, log_dir):
    ...
    model.fit(train_data[0], train_y, iter_num=5000, batch_size=100,
              summaries_dir=log_dir, test_x=test_data[0], test_y=test_y,
              summary_interval=40)
    # model.fit(train_data[0], train_y, iter_num=20000, batch_size=100,
...
if __name__ == '__main__':
    ...
    if os.path.isfile(result_pkl_file):
        ...
        result = LearningResult(name='LSTM_RAW', data=h5py_dir, mode='by_week')
    ...
    for i in range(1, len(split_list)):
        ...
        if result.get_record_by_key(test_name) is None:
            ...
            prediction = load_and_test(test_name, test_set_data, num_classes, result, model=model)
