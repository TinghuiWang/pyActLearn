...
if __name__ == '__main__':
    ...
    sda = SDA(casas_fuel.get_input_dims(), casas_fuel.get_output_dims(), [300, 300, 300])
    ...
    for i in range(1, len(split_list)):
        ...
        if result.get_record_by_key(test_name) is None:
            prediction = training_and_test(test_name, train_set_data, test_set_data, num_classes, result,
                                           model=sda, log_dir=log_dir)
        else:
            prediction = load_and_test(test_name, test_set_data, num_classes, result, model=sda)
